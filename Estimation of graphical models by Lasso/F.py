...
class Graph_Lasso(object):
    ...
    def GRL(self, max_iter=300,lambd=None):
        ...
        return np.around(mle.precision_, decimals=3)
        
def Implement(Cov, lambd, size=200, max_iter=500):
        p = Cov.shape[0]
        np.random.seed(0)
        X = np.random.multivariate_normal(mean=np.zeros(p),
                                         cov=np.linalg.pinv(Cov),
                                         size=size)
        mle =  GraphicalLasso(lambd, max_iter=max_iter).fit(X)
        return (np.around(mle.precision_, decimals=3), X)
